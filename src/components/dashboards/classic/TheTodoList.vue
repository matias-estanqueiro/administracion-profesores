<template>
    <VCard elevation="10" flat class="mb-7 pb-5">
        <v-card-text>
        ...
        <div class="px-5">
            ...
            <v-row class="pb-4">
                ...
                <v-progress-circular :model-value="progress" color="primary" class="mx-2"></v-progress-circular>
            </v-row>
        </div>
                    ...
                    <v-list-item :key="`${i}-${task.text}`" class="d-block">
                        <div class="d-flex align-center w-100">
                            <v-list-item-action class="d-flex align-center w-100">
                                ...
                                <v-chip size="x-small" class="ml-auto" v-if="task.badgebg" :color="task.badgebg">{{
                                    task.badgetext
                                }}</v-chip>
                            </v-list-item-action>
                        ...
                        <div :class="[task.assigntoperson]">
                            <div class="assignedto d-flex ml-7 mb-3 pl-2">
                                <div class="mr-2" v-for="tagbtn in task.tagbtns" :tagbtn="tagbtn" :key="tagbtn.tagbtntitle">
                                    <v-chip color="primary">
                                        <v-avatar>
                                            <v-img :src="`/assets/images/users/${tagbtn.userimg}`" class="mr-2"></v-img>
                                        </v-avatar>
                                        {{ tagbtn.username }}
                                    </v-chip>
                                </div>
